    ...
    public class PrivateKeyFile : IDisposable
    {
        ...
        private Key ParseOpenSshV1Key(byte[] keyFileData, string passPhrase)
        {
            ...
            if (checkInt1 != checkInt2)
                throw new SshException("The checkints differed, the openssh key was not correctly decoded." + checkInt1 + " <->" + checkInt2);
            ...
            switch (keyType)
            {
                case "ssh-ed25519":
                    //public key
                    publicKey = privateKeyReader.ReadBignum2().ToByteArray().Reverse();

                    //private key
                    unencryptedPrivateKey = privateKeyReader.ReadBignum2().ToByteArray().Reverse();
...
#endif
                case "ssh-rsa":
                    var modulus = privateKeyReader.ReadBignum2(); //n
                    var exponent = privateKeyReader.ReadBignum2(); //e
                    var d = privateKeyReader.ReadBignum2(); //d
                    var inverseQ = privateKeyReader.ReadBignum2(); // iqmp
                    var p = privateKeyReader.ReadBignum2(); //p
                    var q = privateKeyReader.ReadBignum2(); //q
